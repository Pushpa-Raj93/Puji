...
def run_demo():
    ...
    print(f"Admin Registration: {resp.json['message']}")
    
    # Register Charity
    resp = client.post('/register', json={'username': 'SaveTheKids', 'password': 'pwd', 'role': 'Charity', 'email': 'charity@example.com', 'mobile': '9876543210'})
    print(f"Charity Registration: {resp.json['message']}")
    
    # Register Donor
    resp = client.post('/register', json={'username': 'Alice', 'password': 'pwd', 'role': 'Donor', 'email': 'alice@example.com', 'mobile': '1234567890'})
    print(f"Donor Registration: {resp.json['message']}")
    
    print_header("2. AUTHENTICATION & JWT")
    # Login Donor
    auth_resp = client.post('/login', json={'username': 'Alice', 'password': 'pwd'})
    donor_token = auth_resp.json.get('token')
    print(f"Alice Logged In. JWT: {donor_token[:20]}... (truncated)")

    # Login Charity
    auth_resp = client.post('/login', json={'username': 'SaveTheKids', 'password': 'pwd'})
    charity_token = auth_resp.json.get('token')
    ...
    admin_token = auth_resp.json.get('token')
    ...
    print(f"Request Status: {resp.json['message']}")
    print(f"Smart Contract Logs: {resp.json['smart_contract_logs']}")
    
    print_header("4. DONOR DONATES FUNDS")
    # Alice donates $600
    headers = {'Authorization': f'Bearer {donor_token}'}
    resp = client.post('/donate', json={'charity': 'SaveTheKids', 'amount': 600}, headers=headers)
    print(f"Donation Status: {resp.json['message']}")
    print(f"Transaction: {resp.json.get('transaction')}")
    print(f"Smart Contract Logs: {resp.json['smart_contract_logs']}") 
    ...
    print(f"Verification Result: {resp.json['message']}")
    
    print_header("7. SIMULATE HACKING ATTACK (TAMPERING)")
    resp = client.post('/tamper_demo')
    print(f"Attack Status: {resp.json['message']}")
    print(f"Tampered Block Index: {resp.json['block_index']}")
    print(f"Tampered Data: {resp.json['new_data_sample']}")
    ...
    print(f"OTP Verification: {verify_resp.json['message']}")
    
    if verify_resp.json['success']:
        # Save
        headers = {'Authorization': f'Bearer {donor_token}'}
        bank_data = {
        ...
        print(f"Save Bank Details: {save_resp.json['message']}")
        
        # Check Profile
        prof_resp = client.get('/get_user_profile', headers=headers)
        print(f"Updated Profile Bank Data: {prof_resp.json['bank_details']}")
